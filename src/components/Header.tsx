    ...
    <header className="sticky top-0 z-50 border-b border-border/40 bg-card/95 backdrop-blur-md">
      ...
      <div className="hidden border-b border-border/30 bg-primary md:block">
        <div className="container-wide flex items-center justify-end gap-6 py-2 text-xs text-primary-foreground/90">
          <a href="tel:+910000000000" className="flex items-center gap-1.5 transition-opacity hover:opacity-80">
            <Phone size={12} /> +91-XXXXX XXXXX
          </a>
          <a
        ...
        <nav className="hidden items-center gap-1 md:flex">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`nav-link rounded-md px-4 py-2 ${
      ...
      {/* Mobile nav */}
      {mobileOpen && (
        <nav className="border-t border-border/30 bg-card px-4 pb-4 md:hidden">
          {navItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              ...
              className={`block rounded-md px-4 py-3 text-sm font-medium ${
                location.pathname === item.path ? "text-accent bg-accent/5" : "text-foreground/80"
              }`}
            >
              {item.label}
            </Link>
